/**
 * seed-inicio-mel.js — v2
 * Cria o Módulo Início completo na Rota do Mel.
 * Campos corretos: parent (não folder), title (não name), body (reading), grade (não gradeCheck)
 *
 * Uso: node scripts/seed-inicio-mel.js
 */
...
function getId(result) {
    ...
    throw new Error('Sem _id na resposta: ' + JSON.stringify(result).substring(0,200));
}
...
function listenAndOrder(speechText, question, orderedItems) {
    return {
        ...
        speechText,
        extra: { speechText, ttsLang: 'pt-BR' },
        choices: orderedItems.map((text, i) => ({
            label: String.fromCharCode(65 + i),
            answer: text,
            ...G1
        }))
...
function matching(question, pairs) {
    ...
    return {
        ...
        question,
        choices: [],
        model: { matching: { left, right, solutions } }
    };
}
...
function dragAndDrop(question, sentence, wordPool, correctOrder) {
    const targets = correctOrder.map((word, i) => ({
        id: `t${i+1}`,
        text: `[${i+1}]`,
        correctOptionId: `w${wordPool.indexOf(word) + 1}`
    }));
    ...
    return {
        ...
        question,
        choices: [],
        model: { dragDropText: { sentence, targets, optionsPool } }
    };
}
...
async function main() {
    console.log('🐝 Seed — Rota do Mel: Módulo Início (v2)');
    console.log('==========================================\n');

    const moduleId = await createFolder('Início', 'module', SUBJECT_ID);
    console.log('');
    ...
    console.log('\n\n✅ Módulo Início — Rota do Mel criado!');
